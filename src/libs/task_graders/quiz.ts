...
function isArrayAnswerEmpty(answers: any) {
...
function testMultiple(valid: any, answers: any) {
    const mistakes = [];
    for(let i=0; i<answers.length; i++) {
...
function testStrict(valid: any, answers: any) {
    const mistakes = [];
    for(let i=0; i<answers.length; i++) {
    ...
    try {
        ...
        for (let i = 0; i < grader_data.length; i++) {
            ...
            if (typeof grader === "function") {
                ...
                const test = testMultiple(grader, answer[i]);
                isValid = test === true;
                ...
                if(Array.isArray(grader.value)) {
                    ...
                    let test: any;
                    if(grader.strict) {
                        test = testStrict(grader.value, answer[i]);
                    } else {
                        test = testMultiple(grader.value, answer[i]);
                    }
                    isValid = test === true;
                    const mistakes = isValid ? [] : test;
        ...
        const nb_total = grader_data.length;
        if(score_settings) {
...
export default {
    gradeAnswer(data: any, answer: any, versions: any, score_settings: ScoreSettings) {
        const grader_data = safeEval(data) // TODO: may be safeEval in repo?
